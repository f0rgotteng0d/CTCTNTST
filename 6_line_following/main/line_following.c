...
#define LSA_MAP_LOW 0
#define LSA_MAP_HIGH 1000
#define BLACK_BOUNDARY 450 // Threshold: values BELOW this are black, ABOVE are white
const int SENSOR_WEIGHTS[NUM_LSA_SENSORS] = {-5, -3, 1, 3, 5};
...
// End-of-Line detection
#define REQUIRED_WHITE_COUNT 10 // Reduced from 25 for faster response
...
// Maneuver settings for box pushing
#define PUSH_SPEED OPTIMUM_DUTY_CYCLE
#define REVERSE_SPEED OPTIMUM_DUTY_CYCLE
#define REVERSE_DURATION_MS 500
#define UTURN_SPEED HIGHER_DUTY_CYCLE
#define UTURN_DURATION_MS 500
#define FORWARD_AFTER_UTURN_MS 500
...
static void process_lsa_readings(line_sensor_array *readings);
static bool check_end_of_line(line_sensor_array *readings, RobotState *state);
static bool check_all_sensors_white(line_sensor_array *readings);
static void update_robot_state_and_error(line_sensor_array *readings, RobotState *state);
...
/**
 * @brief Checks if all sensors are on white (HIGH values > BLACK_BOUNDARY).
 * For sensors where white = ~800, we check if values are ABOVE threshold.
 * @return true if the end of the line is confirmed, false otherwise.
 */
static bool check_end_of_line(line_sensor_array *readings, RobotState *state) {
    bool all_white = true;
    for (int i = 0; i < NUM_LSA_SENSORS; i++) {
        // CHANGED: Now checking <= instead of >= 
        // White values are HIGH (~800), black values are LOW (~200)
        if (readings->adc_reading[i] <= BLACK_BOUNDARY) {
            all_white = false;
            break;
        }
    ...
    if (all_white) {
        state->white_line_count++;
        // Debug logging
        if (state->white_line_count % 5 == 0) {
            ESP_LOGI(TAG, "White count: %d / %d", state->white_line_count, REQUIRED_WHITE_COUNT);
        }
    } else {
        state->white_line_count = 0;
    }
...
/**
 * @brief Checks if all 5 LSA sensors are detecting white (ABOVE BLACK_BOUNDARY).
 * For inverted sensors: white = high values (~800), black = low values (~200)
 * @return true if all sensors are white, false otherwise.
 */
static bool check_all_sensors_white(line_sensor_array *readings) {
    for (int i = 0; i < NUM_LSA_SENSORS; i++) {
        // CHANGED: Now checking <= instead of >=
        // If any sensor reads LOW (black), return false
        if (readings->adc_reading[i] <= BLACK_BOUNDARY) {
            return false; // At least one sensor is on black (low value)
        }
    }
    return true; // All sensors are white (high values)
}
...
static void update_robot_state_and_error(line_sensor_array *readings, RobotState *state) {
    ...
    // CHANGED: Since white is HIGH, black detection uses <= instead of >
    bool left_reading = readings->adc_reading[0] <= BLACK_BOUNDARY;
    bool right_reading = readings->adc_reading[4] <= BLACK_BOUNDARY;


    for (int i = 0; i < NUM_LSA_SENSORS; i++) {
        // CHANGED: Black detection now uses <= instead of >
        if (readings->adc_reading[i] <= BLACK_BOUNDARY) {
            int k = 1;
            weighted_sum += (float)(SENSOR_WEIGHTS[i]) * k;
            sum += k;
    ...
    // Update turn detection flags
    state->far_left_detected = left_reading;
    state->far_right_detected = right_reading && !left_reading;
...
static void execute_motor_control(line_sensor_array *readings, RobotState *state, Peripherals *periph) {
    ...
    if (readings->adc_reading[0] <= BLACK_BOUNDARY &&
        ...
        ESP_LOGI(TAG, "Special pattern - moving straight");
        set_motor_speed(periph->motor_left, MOTOR_FORWARD, state->left_duty_cycle);
        set_motor_speed(periph->motor_right, MOTOR_FORWARD, state->left_duty_cycle);
    }
    // Case 2: Hard Left Turn (Sensor 0)
    else if (state->far_left_detected) {
        ESP_LOGI(TAG, "Hard left turn");
    ...
    // Case 3: Right Turn (Sensor 4)
        ...
        if (readings->adc_reading[2] <= BLACK_BOUNDARY) { // CHANGED: <= instead of >
            // Gentle right (center sensor also on)
            ESP_LOGI(TAG, "Gentle right");
            set_motor_speed(periph->motor_left, MOTOR_FORWARD, state->left_duty_cycle);
            set_motor_speed(periph->motor_right, MOTOR_FORWARD, state->right_duty_cycle);
...
void line_follow_task(void *arg) {
    ...
    ESP_LOGI(TAG, "Line follower started with inverted sensor logic (white=HIGH)");
    ...
    while (true) {
        ...
        // Optional: Add periodic sensor debugging
        static int log_counter = 0;
        if (log_counter++ % 100 == 0) {
            ESP_LOGD(TAG, "LSA: [%d, %d, %d, %d, %d] | Obstacle: %d | White_cnt: %d", 
                     readings.adc_reading[0], readings.adc_reading[1], 
                     readings.adc_reading[2], readings.adc_reading[3], 
                     readings.adc_reading[4], obstacle_present, state.white_line_count);
        }
        ...
        if (obstacle_present) {
            ...
            // Check if all sensors are white (destination reached)
            bool all_white = check_all_sensors_white(&readings);
            
            if (all_white) {
                // TASK: DESTINATION REACHED (Pushed box into pink square)
                ESP_LOGI(TAG, "=== BOX DELIVERED! All sensors white while pushing ===");
                ...
                // 4. Move forward to reacquire line
                ESP_LOGI(TAG, "Moving forward to find line...");
                ...
                vTaskDelay(FORWARD_AFTER_UTURN_MS / portTICK_PERIOD_MS);
                
                // 5. Reset state for clean line following
                init_robot_state(&state);
                ESP_LOGI(TAG, "Maneuver complete. Resuming line following to end zone.");
            ...
            /****************************************
             * STATE 2: END ZONE REACHED (NO OBSTACLE)
             ****************************************/
            stop_motors(&periph);
            ESP_LOGI(TAG, "==== END ZONE REACHED! Bot stopping permanently ====");
            ESP_LOGI(TAG, "Mission complete. White count: %d", state.white_line_count);
            break; // Exit main loop - mission complete
            ...
            ESP_LOGV(TAG, "Error: %.2f | Corr: %.2f | L: %.0f | R: %.0f",
                     state.error, correction, state.left_duty_cycle, state.right_duty_cycle);
        }
    ...
    // 3. Cleanup
    ESP_LOGI(TAG, "Task ending. Motors stopped.");
    vTaskDelete(NULL);
}
